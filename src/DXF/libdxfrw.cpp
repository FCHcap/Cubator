...
bool dxfRW::read(DRW_Interface *interface_, bool ext){
    ...
    ifstream filestr;
    DBG("dxfRW::read 1def\n");
    filestr.open (filepath.c_str(), ios_base::in | ios::binary);
    if (!filestr.is_open())
        return isOk;
    if (!filestr.good())
        return isOk;
    ...
    if (strcmp(line, line2) == 0) {
        ...
        binary = false;
        filestr.open (filepath.c_str(), ios_base::in);
...
void dxfRW::setFilepath(const char *name){
    filepath = name;
}
...
bool dxfRW::write(DRW_Interface *interface_, DRW::Version ver, bool bin){
    ...
    iface = interface_;
    if (binary) {
        filestr.open (filepath.c_str(), ios_base::out | ios::binary | ios::trunc);
        //write sentinel
        filestr << "AutoCAD Binary DXF\r\n" << (char)26 << '\0';
        writer = new dxfWriterBinary(&filestr);
        DBG("dxfRW::read binary file\n");
    } else {
        filestr.open (filepath.c_str(), ios_base::out | ios::trunc);
        ...
        while (reader->readRec(&code, !binary)) {
            ...
            if (code == 0) {
                ...
                if (sectionstr == "SECTION") {
                    ...
                    DBG(code); DBG(" processDxf\n");
                    if (!more) throw CubException(BRIEF24, ERROR23, "DxfRw::processDxf");
